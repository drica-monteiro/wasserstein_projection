...
def Cpt_Suff(S,Y_pred,Y_true,w=1 ,alpha=0.05, boxplot=False, wedge=False):
    '''
    Calculate the Sufficiency index and possibly return the wanted visualisation
    input:
        S: sensible variable
        Y_pred: predicted class
        Y_true: true value to predict
    ...
    assert 0<alpha<2, 'alpha is the argument signifying the quartile, please let it be strictly between 0 and 2'
    assert n==Y_true.shape[0],'every data must have the same length'
    ...
    return _Asymptotic_behavior(Z_EoO,n,alpha, 'Suff', boxplot, wedge)
